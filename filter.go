...
func Filter(enumerable, fun interface{}) interface{} {
	...
	switch enums.Kind() {
	case reflect.Slice:
		return filterOnSlice(enums, fv)
	case reflect.Map:
		return filterOnMap(enums, fv)
	default:
		panic(fmt.Sprintf("%s does not support Filter", enums.Type()))
	}
}

func filterOnSlice(enums, fv reflect.Value) interface{} {
	et := enums.Type().Elem()
...
func filterOnMap(enums, fv reflect.Value) interface{} {
	kt := enums.Type().Key()
	et := enums.Type().Elem()
	mapType := enums.Type()
